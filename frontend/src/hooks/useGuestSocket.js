// frontend/src/hooks/useGuestSocket.js
// Custom hook for guest WebSocket connection to trip chat
...
/**
 * Custom hook for guest real-time trip chat using WebSocket
 * Uses session token instead of Clerk auth
 * @param {string} tripId - The ID of the trip to join
 * @param {Object} guestSession - Guest session info { sessionToken, guestName, memberId }
 * @returns {Object} Socket state and methods
 */
export function useGuestSocket(tripId, guestSession) {
    ...
    useEffect(() => {
        ...
        if (!tripId || !guestSession?.sessionToken) {
            return;
        }

        const connectSocket = () => {
            try {
                // Create socket connection with guest token
                const newSocket = io(API_URL, {
                    auth: { token: `guest:${guestSession.sessionToken}` },
                ...
                newSocket.on('joined-trip', ({ tripId: joinedTripId, activeUsers: users }) => {
                    if (mounted) {
                        console.log(`👥 Guest joined trip ${joinedTripId}, active users:`, users.length);
                ...
                newSocket.on('error', ({ message }) => {
                    if (mounted) {
                        console.error('🔌 Guest socket error:', message);
                        setError(message);
                    }
                });
                ...
                console.error('Failed to connect guest socket:', err);
                if (mounted) {
                    setError(err.message);
                }
            }
    ...
    }, [tripId, guestSession?.sessionToken]);
