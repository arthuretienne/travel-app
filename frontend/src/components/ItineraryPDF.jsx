...
function PageFooter({ city }) {
  return (
    <View style={styles.pageFooter} fixed>
      <Text>skusku.life — Votre voyage, déjà pensé</Text>
  ...
  return (
    <Page size="A4" style={styles.coverPage}>
      <View style={styles.coverBadge}>
        <Text>SKUSKU — PLAN DE VOYAGE</Text>
      </View>
      ...
      {userName && (
        <Text style={styles.coverTraveler}>Préparé pour {userName}</Text>
      )}
...
function TravelDetailsPage({ trip, city }) {
  ...
  return (
    <Page size="A4" style={styles.page}>
      <Text style={styles.sectionHeader}>Détails du voyage</Text>
        ...
        <>
          <View style={styles.infoCard}>
            <View style={styles.infoCardHeader}>
              <Text style={styles.infoCardTitle}>Vol aller</Text>
            </View>
            {flightDetails.outbound?.carrier && (
              <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>Compagnie</Text>
          ...
          {flightDetails.return && (
            <View style={styles.infoCard}>
              <View style={styles.infoCardHeader}>
                <Text style={styles.infoCardTitle}>Vol retour</Text>
              </View>
      ...
      {hotels && hotels.length > 0 && (
        <>
          <Text style={[styles.sectionHeader, { marginTop: 16 }]}>Hébergement</Text>
          {hotels.slice(0, 3).map((hotel, idx) => (
            <View key={idx} style={styles.infoCard}>
              <View style={styles.infoCardHeader}>
                <Text style={styles.infoCardTitle}>{safeText(hotel.name) || `Option hôtel ${idx + 1}`}</Text>
              </View>
              {hotel.neighborhood && (
                <View style={styles.infoRow}>
      ...
      {pricing && (
        <>
          <Text style={[styles.sectionHeader, { marginTop: 16 }]}>Récapitulatif de budget</Text>
          <View style={styles.infoCard}>
            {pricing.flight && (
              <View style={styles.infoRow}>
      ...
      <View style={styles.dayHeader}>
        <View>
          <Text style={styles.dayTitle}>
            {isFirstDay ? 'Jour 1 — Arrivée' : isLastDay ? `Jour ${day.day} — Départ` : `Jour ${day.day}`}
      ...
      {isFirstDay && flightDetails?.outbound && (
        <View style={[styles.activity, { backgroundColor: TEAL_LIGHT, borderRadius: 6, marginBottom: 6 }]}>
          <View style={styles.activityTime}>
            <Text style={styles.activityTimeText}>{safeText(flightDetails.outbound.arrivalTime) || 'À confirmer'}</Text>
      ...
      {day.highlights && day.highlights.length > 0 && (
        <View style={styles.daySummary}>
          <Text style={styles.daySummaryText}>
            <Text style={styles.daySummaryBold}>À ne pas manquer : </Text>
...
function PackingPage({ packing, city }) {
  ...
  const categories = [
    { key: 'essentials', title: 'Essentiels' },
    { key: 'clothing', title: 'Vêtements' },
    { key: 'activityItems', title: 'Pour les activités' },
    { key: 'optional', title: 'Optionnel' },
  ];

  return (
    <Page size="A4" style={styles.page}>
      <Text style={styles.sectionHeader}>Liste de bagages</Text>
