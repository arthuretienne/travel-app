...
- ✅ **Destinations 20 → 38**: +18 Europe court-courrier high-FR-volume (seville, madrid, valencia, palma, naples, venice, florence, milan, berlin, vienna, dublin, edinburgh, krakow, tenerife, faro, nice, brussels, lyon) with unique FR/EN descriptions, real highlights/tips. All auto-prerendered + in sitemap (41 URLs) + Footer + MegaMenu.
- ⚠️ **Remaining (deliberate)**: destinations 38→100 is more content production — infra is additive (add to `DESTINATIONS`; FAQ/budget/sitemap/prerender/menu auto-follow), do NOT mass-generate thin entries. Legal pages need real copy from owner. `VITE_CLERK_PUBLISHABLE_KEY` must be set on Vercel for the build/prerender (else App renders a config-error shell). Future SEO clusters (`/ou-partir/*` etc., Annexe A) must be built before re-adding them to the mega menu.
