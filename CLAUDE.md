...
- ✅ **MegaMenu integrated** (cherry-picked from `claude/sad-bhabha-afa038`): `components/Navigation/MegaMenu.jsx` + `data/megaMenu.js`, mounted on Landing. `megaMenu.js` rewritten **data-driven from `DESTINATIONS`** and **trimmed to existing routes only** (removed all `/ou-partir`, `/outils`, `/itineraires`, `/magazine`, region subpages, `/destination/mexico` — they 404). New destination pages auto-appear in the menu.
- ✅ **Destinations 20 → 52**: +18 Europe court-courrier, then +14 long-courrier (kyoto, seoul, singapore, phuket, hanoi, maldives, mauritius, reunion, cancun, mexico-city, rio-de-janeiro, buenos-aires, miami, montreal). Long-haul batch is **higher quality**: rich ~150-200-word differentiated `descriptionFr` + **bespoke per-destination `faq` array** (5 real Q&A, overrides the generic auto-derived FAQ via `getFaqFr`). New continents `South America` added to `Destinations.jsx` continentOrder + `Footer` CONTINENT_ORDER/FR (else those pages render but stay off the /destinations index). All auto-prerendered (55 URLs) + sitemap + Footer + MegaMenu.
- ⚠️ **Remaining (deliberate)**: 52→100 is more content (infra additive; for quality, prefer bespoke `faq` + long `descriptionFr` like the long-haul batch over the short Europe-batch style — consider back-filling the 38 short ones). Legal pages need real copy from owner. `VITE_CLERK_PUBLISHABLE_KEY` must be set on Vercel for build/prerender. Future SEO clusters (`/ou-partir/*` etc., Annexe A) must exist before re-adding them to the mega menu.
